...
class TfPoseEstimator:
    ...
    def __init__(self, graph_path, target_size=(320, 240), tf_config=None,manual_roi=(0,0)):
    ...
    @staticmethod
    def draw_humans(npimg,roi,start_pos, humans,dual_cam = False, imgcopy=False):
        a=start_pos[1];b=start_pos[0]
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = roi.shape[:2]
        centers = {}
        for human in humans:
            # draw point
            for i in range(common.CocoPart.Background.value):
                ...
                conf = body_part.score

                # print(body_part)
                center     =  (int(body_part.x * image_w +a+ 0.5),
                                           int(body_part.y * image_h +b+ 0.5))
                # print(center)
                centers[i] = center
                if conf < 0.4:
                    centers[i] = (-1,-1)
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                if pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                    continue
                if centers[pair[0]] == (-1,-1):
                    continue
                if centers[pair[1]] ==(-1,-1):
                    continue
